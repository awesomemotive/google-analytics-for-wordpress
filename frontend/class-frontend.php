	...
	class GA_Filter {
		...
		function not_shown_error() {
			if ( current_user_can( 'manage_options' ) )
				echo "<!-- " . __( "Google Analytics tracking code not shown because you haven't setup Google Analytics for WordPress yet.", 'google-analytics-for-wordpress' ) . " -->\n";
		}
                ...
                ga.src = <?php
					if ( $this->options['gajslocalhosting'] && !empty( $this->options['gajsurl'] ) ) {
						...
						if ( current_user_can( 'manage_options' ) && $this->options['debug'] )
							$script = 'dc.js';
							//Unsure whether the old debug script is supported.
							//Instead I'm using 'https://chrome.google.com/webstore/detail/google-analytics-debugger/jnkmfdileelhofjcijamephohjechhna'
