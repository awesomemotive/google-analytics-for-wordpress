...
class MonsterInsights_GA_Lib_IO_Curl extends MonsterInsights_GA_Lib_IO_Abstract
{
  ...
  public function __construct(MonsterInsights_GA_Lib_Client $client)
  {
    ...
    $this->disableProxyWorkaround = $this->client->getClassConfig(
        'MonsterInsights_GA_Lib_IO_Curl',
  ...
  public function executeRequest(MonsterInsights_GA_Lib_Http_Request $request)
  {
    ...
    $options = $this->client->getClassConfig('MonsterInsights_GA_Lib_IO_Curl', 'options');
    if (is_array($options)) {
    ...
    if ($response === false) {
      ...
      $map = $this->client->getClassConfig('MonsterInsights_GA_Lib_IO_Exception', 'retry_map');

      $this->client->getLogger()->error('cURL ' . $error);
      throw new MonsterInsights_GA_Lib_IO_Exception($error, $code, null, $map);
    }
  ...
  protected function needsQuirk()
  {
    ...
    return $versionNum < MonsterInsights_GA_Lib_IO_Curl::NO_QUIRK_VERSION;
  }
}
