	...
	class Yoast_GA_Admin extends Yoast_GA_Options {
		...
		public function save_settings( $data ) {
			...
			foreach ( $defaults[$this->option_prefix] as $key => $value ) {
				if ( ! isset( $data[$key] ) ) {
					// If no data was passed in and the option has no value, set it to the default.
					if ( ! isset( $this->options[$key] ) ) {
						$this->options[$key] = $value;
					}
					// If no data was passed and the option was originally true, set it to false.
					elseif ( $this->options[$key] === '1' ) {
						$this->options[$key] = '0';
					}
				}
			}
